...
be one. For all application we will discuss in this article, $m(\mu,ij)$ will be one or zero.
...
manipulation of complicated (sums of) functions by set theoretic considerations on graphs.

Note that the label of a labelled point is not the property $z_i$ but the fact that a fixed variable $\bo r$
is assigned to that point. For brevity, the property is often omitted when it is clear from the context.
The same applies to bonds. Sometimes different graphical representations are used to distinguish between
bond types.
...
\begin{enumerate}[i.]
  ...
  \begin{tikzpicture}[baseline={([yshift=-.7em]current bounding box.center)}]%current bounding box.center
  	\draw (1,0) node[circle,minimum size=0.2cm,draw,fill=black] (A) {};
	\node [above left] at (A) {$z_1$};
	\end{tikzpicture}
= \int_\Om \dr_1 z_1(\r_1) .
  ...
  \begin{tikzpicture}[baseline={(current bounding box.center)}]%current bounding box.center
	...
	\node [above left] at (B) {$z_2$};
	\end{tikzpicture}
= \int_\Om \dr_2 \int_\Om \dr_1 z_1(\r_1) f_\mu(\r_1,\r_2) z_2(\r_2).
\label{2fields}
}
 \item And the general version:\vskip5pt%\newline
  \{A graph with $n$ labelled points and $m$ field points and\\* \vphantom{}\hskip5pt  arbirtrary bonds\}\\
  \itemEq{\begin{aligned}
  = \int_\Om &\dr_{n+m} \cdots \int_\Om \dr_{n+1} \times \{\te{the same graph with all}\\ &\te{field points carrying label $\r_{n+1}$ to $\r_{n+m}$}\}.
...
two graphs with labelled (field) points: two graphs are \ep{topologically indistinguishable}
...
% Just as an example, consider the expression
% 
% \begin{equation}
% \begin{aligned}
% \int_{\Om^3} &\dr^3\ z(\r_1)z(\r_2)z(\r_3)(1+f(\r_1,\r_2))(1+f(\r_1,\r_3)) \\
% &=\int_{\Om^3} \dr^3\ z(\r_1)z(\r_2)z(\r_3) \\
%  &\times \lt[1+f(\r_1,\r_2)+f(\r_1,\r_3)+f(\r_1,\r_2)f(\r_1,\r_3)\rt] \\
% %&+\qquad \int_{\Om^3} \dr^3 f(\r_1,\r_2))(1+f(\r_1,\r_3))
% \end{aligned}
% \end{equation} 
...
% The above definition is not a uniformly used definition in graph theory. Some authors
% distinguish between graphs with labelled field points and graphs with unlabelled field points
% (cf. definitions 3 and 3' of Hiroike & Morita\cite{Hiroike}), leading to equations such as
% \begin{equation}
% \begin{tikzpicture}[baseline={(current bounding box.center)}]%current bounding box.center
%   	\draw (0,0) node[circle,minimum size=0.2cm,draw,fill=black] (A) {};
%   	\draw (1.5,0) node[circle,minimum size=0.2cm,draw,fill=black] (B) {};
...
% 	\node [below right] at (B) {$\r_2$};
% 	\end{tikzpicture}.
% \label{nolabel:label:1}
% \end{equation}
% We chose not to follow this convention. This is in agreement with Wertheim \cite{Wertheim1}. It
% also seems more intuitive to not distinguish between labelled and unlabelled integration
% variables since from a mathematical standpoint, the result of an integration is
% entirely independent of the name of the integration variables as long as they are unique and well-defined.
% 
% So, our version of \eqref{nolabel:label:1} is simply
% \begin{equation}
% \begin{tikzpicture}[baseline={(current bounding box.center)}]%current bounding box.center
%   	\draw (0,0) node[circle,minimum size=0.2cm,draw,fill=black] (A) {};
%   	\draw (1.5,0) node[circle,minimum size=0.2cm,draw,fill=black] (B) {};
...
% 	\node [below right] at (B) {$\r_2$};
% 	\end{tikzpicture}.
% \label{nolabel:label:2}
% \end{equation}
% The difference between \eqref{nolabel:label:1} and \eqref{nolabel:label:2} is the factor $\frac 1 2$,
% which is the \ep{symmetry number s} of the graph. It is defined as the number of possible
% permutations of graph field point labels which do not change the topology of the graph. Two graphs
% have the same topology if field points with the same label $i$ carry the same
% properties and pairs of points with labels $i$ and $j$ are linked by the same bonds in both graphs.
% 
% Obviously, for a graph with $N$ field points $1 \le s \le N!$. In the example of \eqref{nolabel:label:1},
% the identity and the permutation of labels 1 and 2 results in two topologically indistinguishable graphs,
% so the symmetry number is $s=2$. We will explore the usefulness of symmetry numbers in the next section.
...
%, \qquad u^{(2)}(12)=\rho^{(2)}(12)-\rho(1)\rho(2).
...
graphs vanish later in the expansion. A classical example is the \ep{hard sphere potential}
...
in $\dc$
%% CHECK: better idea: first graphs with an attraction bond between only two points.
\newcommand\twograph[2]{

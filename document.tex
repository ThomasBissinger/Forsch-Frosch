...
\twocolumn[
  ...
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
      ...
      simulation results to obtain more accurate approximations than with the former
      fit. 
\\
\\
    \end{abstract}
...
$\dc$ with a single $s$-meric \ep{chain} for $s\ge2$. By chain we mean singly
...
$\nu$ independence, $\lanu$. We call it simply the \ep{$\nu$ dependent} function. Since
...
\id\textbf{Residual definition.} We want to find the optimum $\la'$ with respect to $\lao'$. 
But the optimum needs further definition. There are of course numerous possibilites
for the fitting function, of which we chose a simple \ep{least squares fit}, that
is we try to minimize the \ep{root mean square deviation} (RMS) 
...
functions. Just a quick word on nomenclature: we will call the $\nu$ fix functions
...
however, is more problematic. It is possible that this also amounts from the increasing
oscillations toward higher $\e$. When we consider the coefficients, we can see that
